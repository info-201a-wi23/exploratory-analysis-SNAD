...
### Summary Information
The variables we looked into were the total number of countries in each region, the average Ladder score, the average score of freedom to make life choices, the average GDP per capita score, and the average score for a healthy life expectancy. For one comparison, we compared the ladder score to each country to see which one had the highest or lowest overall happiness score- with Finland scoring the overall highest. Another connection we observed was between the life expectancy in each community, which is an overall term to measure how healthy an individual is likely to be- and here, top 3 regions that scored the highest life expectancy score was East Asia, North America, and Western Europe. 
...
```{r, echo = FALSE}
knitr::include_graphics(file.path(getwd(), "chart_1.png"))
```
...
knitr::include_graphics(file.path(getwd(), "chart_2.png"))
```
...
```{r, echo = FALSE}
knitr::include_graphics(file.path(getwd(), "chart_3.png"))
```
